...
class SoundTest {
	constructor() {
		...
		this.fm = {addr: 0, reg: new Uint8Array(0x100), kon: new Array(8).fill(false)};
		...
		this.cpu.memorymap[0x20].read = addr => addr === 0x2001 ? 0 : 0xff;
		this.cpu.memorymap[0x20].write = (addr, data) => {
			switch (addr & 0xff) {
			case 0:
...
function success(zip) {
	PRG2 = new Uint8Array(zip.files['sk3_4.6b'].inflate().split('').map(c => c.charCodeAt(0))).addBase();
