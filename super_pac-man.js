...
class SuperPacMan {
	constructor() {
		...
		for (let i = 0; i < 0x40; i++)
			this.cpu[0].memorymap[0xc0 + i].base = PRG1.base[i];
		for (let i = 0; i < 4; i++) {
	...
	updateInput() {
		...
		if (this.fCoin && this.dwCoin < 150 && ++this.dwCoin > 99)
			this.dwCoin = 99;
		if (this.fStart1P && !this.ram[0x2409] && this.dwCoin > 0 && this.dwCoin < 150)
			--this.dwCoin;
		if (this.fStart2P && !this.ram[0x2409] && this.dwCoin > 1 && this.dwCoin < 150)
			this.dwCoin -= 2;
		this.fCoin = this.fStart1P = this.fStart2P = false;

		if (!this.fPortTest) {
			let i, p;
			this.ram.set(this.port.subarray(2, 8), 0x2402);
			this.ram.set(this.port.subarray(0x10, 0x18), 0x2410);
			switch (this.ram[0x2408] & 0x0f) {
			case 1:
				this.ram[0x2400] = this.port[5];
			...
			case 2:
				...
				break;
			case 4:
				this.ram[0x2400] = this.dwCoin / 10;
				this.ram[0x2401] = this.dwCoin % 10;
				break;
			case 8:
	...
	xfer8x8(data, p, k) {
		const q = ((this.ram[k + 0x400] << 8 | this.ram[k]) & 0x3fff) << 6;
