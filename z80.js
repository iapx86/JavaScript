/*
 *
 *	Z80 Emulator
 *
 */

import Cpu, {dummypage} from './main.js';

export default class Z80 extends Cpu {
	constructor(arg = null) {
		super(arg);
		this.b = 0;
		this.c = 0;
		this.d = 0;
		this.e = 0;
		this.h = 0;
		this.l = 0;
		this.a = 0;
		this.f = 0; // f:sz-h-pnc
		this.ixh = 0;
		this.ixl = 0;
		this.iyh = 0;
		this.iyl = 0;
		this.iff = 0;
		this.im = 0;
		this.i = 0;
		this.r = 0;
		this.b_prime = 0;
		this.c_prime = 0;
		this.d_prime = 0;
		this.e_prime = 0;
		this.h_prime = 0;
		this.l_prime = 0;
		this.a_prime = 0;
		this.f_prime = 0;
		this.sp = 0;
		this.iomap = [];
		for (let i = 0; i < 0x100; i++)
			this.iomap.push({base: dummypage, read: null, write: () => {}});
	}

	reset() {
		super.reset();
		this.iff = 0;
		this.im = 0;
		this.sp = 0;
		this.pc = 0;
	}

	interrupt(vector = 0xff) {
		let v;

		if (!super.interrupt() || this.iff !== 3)
			return false;
		this.iff = 0;
		switch (this.im) {
		case 0:
			switch (vector) {
			case 0xc7: // RST 00h
				this.rst(0x00);
				break;
			case 0xcf: // RST 08h
				this.rst(0x08);
				break;
			case 0xd7: // RST 10h
				this.rst(0x10);
				break;
			case 0xdf: // RST 18h
				this.rst(0x18);
				break;
			case 0xe7: // RST 20h
				this.rst(0x20);
				break;
			case 0xef: // RST 28h
				this.rst(0x28);
				break;
			case 0xf7: // RST 30h
				this.rst(0x30);
				break;
			case 0xff: // RST 38h
				this.rst(0x38);
				break;
			}
			break;
		case 1:
			this.rst(0x38);
			break;
		case 2:
			this.rst(this.read(v = vector & 0xff | this.i << 8) | this.read1(v) << 8);
			break;
		}
		return true;
	}

	non_maskable_interrupt() {
		if (!super.interrupt())
			return false;
		this.iff &= 2;
		this.rst(0x66);
		return true;
	}

	_execute() {
		let v;

		this.r = this.r + 1 & 0x7f;
		switch (this.fetch()) {
		case 0x00: // NOP
			return;
		case 0x01: // LD BC,nn
			return void([this.c, this.b] = [this.fetch(), this.fetch()]);
		case 0x02: // LD (BC),A
			return void this.write(this.c | this.b << 8, this.a);
		case 0x03: // INC BC
			return void this.incbc();
		case 0x04: // INC B
			return void(this.b = this.inc(this.b));
		case 0x05: // DEC B
			return void(this.b = this.dec(this.b));
		case 0x06: // LD B,n
			return void(this.b = this.fetch());
		case 0x07: // RLCA
			return void this.rlca();
		case 0x08: // EX AF,AF'
			return void([this.a, this.f, this.a_prime, this.f_prime] = [this.a_prime, this.f_prime, this.a, this.f]);
		case 0x09: // ADD HL,BC
			this.l = (v = Z80.aAdd[0][this.c][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.b][this.h]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x0a: // LD A,(BC)
			return void(this.a = this.read(this.c | this.b << 8));
		case 0x0b: // DEC BC
			return void this.decbc();
		case 0x0c: // INC C
			return void(this.c = this.inc(this.c));
		case 0x0d: // DEC C
			return void(this.c = this.dec(this.c));
		case 0x0e: // LD C,n
			return void(this.c = this.fetch());
		case 0x0f: // RRCA
			return void this.rrca();
		case 0x10: // DJNZ e
			return void this.jr((this.b = this.b - 1 & 0xff) !== 0);
		case 0x11: // LD DE,nn
			return void([this.e, this.d] = [this.fetch(), this.fetch()]);
		case 0x12: // LD (DE),A
			return void this.write(this.e | this.d << 8, this.a);
		case 0x13: // INC DE
			return void this.incde();
		case 0x14: // INC D
			return void(this.d = this.inc(this.d));
		case 0x15: // DEC D
			return void(this.d = this.dec(this.d));
		case 0x16: // LD D,n
			return void(this.d = this.fetch());
		case 0x17: // RLA
			return void this.rla();
		case 0x18: // JR e
			return void this.jr(true);
		case 0x19: // ADD HL,DE
			this.l = (v = Z80.aAdd[0][this.e][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.d][this.h]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x1a: // LD A,(DE)
			return void(this.a = this.read(this.e | this.d << 8));
		case 0x1b: // DEC DE
			return void this.decde();
		case 0x1c: // INC E
			return void(this.e = this.inc(this.e));
		case 0x1d: // DEC E
			return void(this.e = this.dec(this.e));
		case 0x1e: // LD E,n
			return void(this.e = this.fetch());
		case 0x1f: // RRA
			return void this.rra();
		case 0x20: // JR NZ,e
			return void this.jr((this.f & 0x40) === 0);
		case 0x21: // LD HL,nn
			return void([this.l, this.h] = [this.fetch(), this.fetch()]);
		case 0x22: // LD (nn),HL
			this.write(v = this.fetch() | this.fetch() << 8, this.l);
			this.write1(v, this.h);
			return;
		case 0x23: // INC HL
			return void this.inchl();
		case 0x24: // INC H
			return void(this.h = this.inc(this.h));
		case 0x25: // DEC H
			return void(this.h = this.dec(this.h));
		case 0x26: // LD H,n
			return void(this.h = this.fetch());
		case 0x27: // DAA
			this.a = (v = Z80.aDaa[this.f >>> 2 & 4 | this.f & 3][this.a]) & 0xff;
			this.f = v >>> 8;
			return;
		case 0x28: // JR Z,e
			return void this.jr((this.f & 0x40) !== 0);
		case 0x29: // ADD HL,HL
			this.l = (v = Z80.aAdd[0][this.l][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.h][this.h]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x2a: // LD HL,(nn)
			this.l = this.read(v = this.fetch() | this.fetch() << 8);
			this.h = this.read1(v);
			return;
		case 0x2b: // DEC HL
			return void this.dechl();
		case 0x2c: // INC L
			return void(this.l = this.inc(this.l));
		case 0x2d: // DEC L
			return void(this.l = this.dec(this.l));
		case 0x2e: // LD L,n
			return void(this.l = this.fetch());
		case 0x2f: // CPL
			this.a ^= 0xff;
			this.f |= 0x12;
			return;
		case 0x30: // JR NC,e
			return void this.jr((this.f & 1) === 0);
		case 0x31: // LD SP,nn
			return void(this.sp = this.fetch() | this.fetch() << 8);
		case 0x32: // LD (nn),A
			return void this.write(this.fetch() | this.fetch() << 8, this.a);
		case 0x33: // INC SP
			return void(this.sp = this.sp + 1 & 0xffff);
		case 0x34: // INC (HL)
			return void this.write(v = this.l | this.h << 8, this.inc(this.read(v)));
		case 0x35: // DEC (HL)
			return void this.write(v = this.l | this.h << 8, this.dec(this.read(v)));
		case 0x36: // LD (HL),n
			return void this.write(this.l | this.h << 8, this.fetch());
		case 0x37: // SCF
			return void(this.f = this.f & 0xc4 | 1);
		case 0x38: // JR C,e
			return void this.jr((this.f & 1) !== 0);
		case 0x39: // ADD HL,SP
			this.l = (v = Z80.aAdd[0][this.sp & 0xff][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.sp >>> 8][this.h]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x3a: // LD A,(nn)
			return void(this.a = this.read(this.fetch() | this.fetch() << 8));
		case 0x3b: // DEC SP
			return void(this.sp = this.sp - 1 & 0xffff);
		case 0x3c: // INC A
			return void(this.a = this.inc(this.a));
		case 0x3d: // DEC A
			return void(this.a = this.dec(this.a));
		case 0x3e: // LD A,n
			return void(this.a = this.fetch());
		case 0x3f: // CCF
			return void(this.f = this.f & 0xc5 ^ 1);
		case 0x40: // LD B,B
			return;
		case 0x41: // LD B,C
			return void(this.b = this.c);
		case 0x42: // LD B,D
			return void(this.b = this.d);
		case 0x43: // LD B,E
			return void(this.b = this.e);
		case 0x44: // LD B,H
			return void(this.b = this.h);
		case 0x45: // LD B,L
			return void(this.b = this.l);
		case 0x46: // LD B,(HL)
			return void(this.b = this.read(this.l | this.h << 8));
		case 0x47: // LD B,A
			return void(this.b = this.a);
		case 0x48: // LD C,B
			return void(this.c = this.b);
		case 0x49: // LD C,C
			return;
		case 0x4a: // LD C,D
			return void(this.c = this.d);
		case 0x4b: // LD C,E
			return void(this.c = this.e);
		case 0x4c: // LD C,H
			return void(this.c = this.h);
		case 0x4d: // LD C,L
			return void(this.c = this.l);
		case 0x4e: // LD C,(HL)
			return void(this.c = this.read(this.l | this.h << 8));
		case 0x4f: // LD C,A
			return void(this.c = this.a);
		case 0x50: // LD D,B
			return void(this.d = this.b);
		case 0x51: // LD D,C
			return void(this.d = this.c);
		case 0x52: // LD D,D
			return;
		case 0x53: // LD D,E
			return void(this.d = this.e);
		case 0x54: // LD D,H
			return void(this.d = this.h);
		case 0x55: // LD D,L
			return void(this.d = this.l);
		case 0x56: // LD D,(HL)
			return void(this.d = this.read(this.l | this.h << 8));
		case 0x57: // LD D,A
			return void(this.d = this.a);
		case 0x58: // LD E,B
			return void(this.e = this.b);
		case 0x59: // LD E,C
			return void(this.e = this.c);
		case 0x5a: // LD E,D
			return void(this.e = this.d);
		case 0x5b: // LD E,E
			return;
		case 0x5c: // LD E,H
			return void(this.e = this.h);
		case 0x5d: // LD E,L
			return void(this.e = this.l);
		case 0x5e: // LD E,(HL)
			return void(this.e = this.read(this.l | this.h << 8));
		case 0x5f: // LD E,A
			return void(this.e = this.a);
		case 0x60: // LD H,B
			return void(this.h = this.b);
		case 0x61: // LD H,C
			return void(this.h = this.c);
		case 0x62: // LD H,D
			return void(this.h = this.d);
		case 0x63: // LD H,E
			return void(this.h = this.e);
		case 0x64: // LD H,H
			return;
		case 0x65: // LD H,L
			return void(this.h = this.l);
		case 0x66: // LD H,(HL)
			return void(this.h = this.read(this.l | this.h << 8));
		case 0x67: // LD H,A
			return void(this.h = this.a);
		case 0x68: // LD L,B
			return void(this.l = this.b);
		case 0x69: // LD L,C
			return void(this.l = this.c);
		case 0x6a: // LD L,D
			return void(this.l = this.d);
		case 0x6b: // LD L,E
			return void(this.l = this.e);
		case 0x6c: // LD L,H
			return void(this.l = this.h);
		case 0x6d: // LD L,L
			return;
		case 0x6e: // LD L,(HL)
			return void(this.l = this.read(this.l | this.h << 8));
		case 0x6f: // LD L,A
			return void(this.l = this.a);
		case 0x70: // LD (HL),B
			return void this.write(this.l | this.h << 8, this.b);
		case 0x71: // LD (HL),C
			return void this.write(this.l | this.h << 8, this.c);
		case 0x72: // LD (HL),D
			return void this.write(this.l | this.h << 8, this.d);
		case 0x73: // LD (HL),E
			return void this.write(this.l | this.h << 8, this.e);
		case 0x74: // LD (HL),H
			return void this.write(this.l | this.h << 8, this.h);
		case 0x75: // LD (HL),L
			return void this.write(this.l | this.h << 8, this.l);
		case 0x76: // HALT
			return void this.suspend();
		case 0x77: // LD (HL),A
			return void this.write(this.l | this.h << 8, this.a);
		case 0x78: // LD A,B
			return void(this.a = this.b);
		case 0x79: // LD A,C
			return void(this.a = this.c);
		case 0x7a: // LD A,D
			return void(this.a = this.d);
		case 0x7b: // LD A,E
			return void(this.a = this.e);
		case 0x7c: // LD A,H
			return void(this.a = this.h);
		case 0x7d: // LD A,L
			return void(this.a = this.l);
		case 0x7e: // LD A,(HL)
			return void(this.a = this.read(this.l | this.h << 8));
		case 0x7f: // LD A,A
			return;
		case 0x80: // ADD A,B
			return void this.add(this.b);
		case 0x81: // ADD A,C
			return void this.add(this.c);
		case 0x82: // ADD A,D
			return void this.add(this.d);
		case 0x83: // ADD A,E
			return void this.add(this.e);
		case 0x84: // ADD A,H
			return void this.add(this.h);
		case 0x85: // ADD A,L
			return void this.add(this.l);
		case 0x86: // ADD A,(HL)
			return void this.add(this.read(this.l | this.h << 8));
		case 0x87: // ADD A,A
			return void this.add(this.a);
		case 0x88: // ADC A,B
			return void this.adc(this.b);
		case 0x89: // ADC A,C
			return void this.adc(this.c);
		case 0x8a: // ADC A,D
			return void this.adc(this.d);
		case 0x8b: // ADC A,E
			return void this.adc(this.e);
		case 0x8c: // ADC A,H
			return void this.adc(this.h);
		case 0x8d: // ADC A,L
			return void this.adc(this.l);
		case 0x8e: // ADC A,(HL)
			return void this.adc(this.read(this.l | this.h << 8));
		case 0x8f: // ADC A,A
			return void this.adc(this.a);
		case 0x90: // SUB B
			return void this.sub(this.b);
		case 0x91: // SUB C
			return void this.sub(this.c);
		case 0x92: // SUB D
			return void this.sub(this.d);
		case 0x93: // SUB E
			return void this.sub(this.e);
		case 0x94: // SUB H
			return void this.sub(this.h);
		case 0x95: // SUB L
			return void this.sub(this.l);
		case 0x96: // SUB (HL)
			return void this.sub(this.read(this.l | this.h << 8));
		case 0x97: // SUB A
			return void this.sub(this.a);
		case 0x98: // SBC A,B
			return void this.sbc(this.b);
		case 0x99: // SBC A,C
			return void this.sbc(this.c);
		case 0x9a: // SBC A,D
			return void this.sbc(this.d);
		case 0x9b: // SBC A,E
			return void this.sbc(this.e);
		case 0x9c: // SBC A,H
			return void this.sbc(this.h);
		case 0x9d: // SBC A,L
			return void this.sbc(this.l);
		case 0x9e: // SBC A,(HL)
			return void this.sbc(this.read(this.l | this.h << 8));
		case 0x9f: // SBC A,A
			return void this.sbc(this.a);
		case 0xa0: // AND B
			return void this.and(this.b);
		case 0xa1: // AND C
			return void this.and(this.c);
		case 0xa2: // AND D
			return void this.and(this.d);
		case 0xa3: // AND E
			return void this.and(this.e);
		case 0xa4: // AND H
			return void this.and(this.h);
		case 0xa5: // AND L
			return void this.and(this.l);
		case 0xa6: // AND (HL)
			return void this.and(this.read(this.l | this.h << 8));
		case 0xa7: // AND A
			return void this.and(this.a);
		case 0xa8: // XOR B
			return void this.xor(this.b);
		case 0xa9: // XOR C
			return void this.xor(this.c);
		case 0xaa: // XOR D
			return void this.xor(this.d);
		case 0xab: // XOR E
			return void this.xor(this.e);
		case 0xac: // XOR H
			return void this.xor(this.h);
		case 0xad: // XOR L
			return void this.xor(this.l);
		case 0xae: // XOR (HL)
			return void this.xor(this.read(this.l | this.h << 8));
		case 0xaf: // XOR A
			return void this.xor(this.a);
		case 0xb0: // OR B
			return void this.or(this.b);
		case 0xb1: // OR C
			return void this.or(this.c);
		case 0xb2: // OR D
			return void this.or(this.d);
		case 0xb3: // OR E
			return void this.or(this.e);
		case 0xb4: // OR H
			return void this.or(this.h);
		case 0xb5: // OR L
			return void this.or(this.l);
		case 0xb6: // OR (HL)
			return void this.or(this.read(this.l | this.h << 8));
		case 0xb7: // OR A
			return void this.or(this.a);
		case 0xb8: // CP B
			return void this.cp(this.b);
		case 0xb9: // CP C
			return void this.cp(this.c);
		case 0xba: // CP D
			return void this.cp(this.d);
		case 0xbb: // CP E
			return void this.cp(this.e);
		case 0xbc: // CP H
			return void this.cp(this.h);
		case 0xbd: // CP L
			return void this.cp(this.l);
		case 0xbe: // CP (HL)
			return void this.cp(this.read(this.l | this.h << 8));
		case 0xbf: // CP A
			return void this.cp(this.a);
		case 0xc0: // RET NZ
			return void this.ret((this.f & 0x40) === 0);
		case 0xc1: // POP BC
			return void([this.c, this.b] = [this.pop(), this.pop()]);
		case 0xc2: // JP NZ,nn
			return void this.jp((this.f & 0x40) === 0);
		case 0xc3: // JP nn
			return void this.jp(true);
		case 0xc4: // CALL NZ,nn
			return void this.call((this.f & 0x40) === 0);
		case 0xc5: // PUSH BC
			this.push(this.b);
			this.push(this.c);
			return;
		case 0xc6: // ADD A,n
			return void this.add(this.fetch());
		case 0xc7: // RST 00h
			return void this.rst(0x00);
		case 0xc8: // RET Z
			return void this.ret((this.f & 0x40) !== 0);
		case 0xc9: // RET
			return void this.ret(true);
		case 0xca: // JP Z,nn
			return void this.jp((this.f & 0x40) !== 0);
		case 0xcb:
			return void this.execute_cb();
		case 0xcc: // CALL Z,nn
			return void this.call((this.f & 0x40) !== 0);
		case 0xcd: // CALL nn
			return void this.call(true);
		case 0xce: // ADC A,n
			return void this.adc(this.fetch());
		case 0xcf: // RST 08h
			return void this.rst(0x08);
		case 0xd0: // RET NC
			return void this.ret((this.f & 1) === 0);
		case 0xd1: // POP DE
			return void([this.e, this.d] = [this.pop(), this.pop()]);
		case 0xd2: // JP NC,nn
			return void this.jp((this.f & 1) === 0);
		case 0xd3: // OUT n,A
			return void this.iowrite(this.a, this.fetch(), this.a);
		case 0xd4: // CALL NC,nn
			return void this.call((this.f & 1) === 0);
		case 0xd5: // PUSH DE
			this.push(this.d);
			this.push(this.e);
			return;
		case 0xd6: // SUB n
			return void this.sub(this.fetch());
		case 0xd7: // RST 10h
			return void this.rst(0x10);
		case 0xd8: // RET C
			return void this.ret((this.f & 1) !== 0);
		case 0xd9: // EXX
			[this.b, this.c, this.b_prime, this.c_prime] = [this.b_prime, this.c_prime, this.b, this.c];
			[this.d, this.e, this.d_prime, this.e_prime] = [this.d_prime, this.e_prime, this.d, this.e];
			[this.h, this.l, this.h_prime, this.l_prime] = [this.h_prime, this.l_prime, this.h, this.l];
			return;
		case 0xda: // JP C,nn
			return void this.jp((this.f & 1) !== 0);
		case 0xdb: // IN A,n
			return void(this.a = this.ioread(this.a, this.fetch()));
		case 0xdc: // CALL C,nn
			return void this.call((this.f & 1) !== 0);
		case 0xdd:
			return void this.execute_dd();
		case 0xde: // SBC A,n
			return void this.sbc(this.fetch());
		case 0xdf: // RST 18h
			return void this.rst(0x18);
		case 0xe0: // RET PO
			return void this.ret((this.f & 4) === 0);
		case 0xe1: // POP HL
			return void([this.l, this.h] = [this.pop(), this.pop()]);
		case 0xe2: // JP PO,nn
			return void this.jp((this.f & 4) === 0);
		case 0xe3: // EX (SP),HL
			v = this.read(this.sp);
			this.write(this.sp, this.l);
			this.l = v;
			v = this.read1(this.sp);
			this.write1(this.sp, this.h);
			this.h = v;
			return;
		case 0xe4: // CALL PO,nn
			return void this.call((this.f & 4) === 0);
		case 0xe5: // PUSH HL
			this.push(this.h);
			this.push(this.l);
			return;
		case 0xe6: // AND n
			return void this.and(this.fetch());
		case 0xe7: // RST 20h
			return void this.rst(0x20);
		case 0xe8: // RET PE
			return void this.ret((this.f & 4) !== 0);
		case 0xe9: // JP (HL)
			return void(this.pc = this.l | this.h << 8);
		case 0xea: // JP PE,nn
			return void this.jp((this.f & 4) !== 0);
		case 0xeb: // EX DE,HL
			return void([this.d, this.e, this.h, this.l] = [this.h, this.l, this.d, this.e]);
		case 0xec: // CALL PE,nn
			return void this.call((this.f & 4) !== 0);
		case 0xed:
			return void this.execute_ed();
		case 0xee: // XOR n
			return void this.xor(this.fetch());
		case 0xef: // RST 28h
			return void this.rst(0x28);
		case 0xf0: // RET P
			return void this.ret((this.f & 0x80) === 0);
		case 0xf1: // POP AF
			return void([this.f, this.a] = [this.pop(), this.pop()]);
		case 0xf2: // JP P,nn
			return void this.jp((this.f & 0x80) === 0);
		case 0xf3: // DI
			return void(this.iff = 0);
		case 0xf4: // CALL P,nn
			return void this.call((this.f & 0x80) === 0);
		case 0xf5: // PUSH AF
			this.push(this.a);
			this.push(this.f);
			return;
		case 0xf6: // OR n
			return void this.or(this.fetch());
		case 0xf7: // RST 30h
			return void this.rst(0x30);
		case 0xf8: // RET M
			return void this.ret((this.f & 0x80) !== 0);
		case 0xf9: // LD SP,HL
			return void(this.sp = this.l | this.h << 8);
		case 0xfa: // JP M,nn
			return void this.jp((this.f & 0x80) !== 0);
		case 0xfb: // EI
			return void(this.iff = 3);
		case 0xfc: // CALL M,nn
			return void this.call((this.f & 0x80) !== 0);
		case 0xfd:
			return void this.execute_fd();
		case 0xfe: // CP A,n
			return void this.cp(this.fetch());
		case 0xff: // RST 38h
			return void this.rst(0x38);
		}
	}

	execute_cb() {
		let v;

		switch (this.fetch()) {
		case 0x00: // RLC B
			return void(this.b = this.rlc(this.b));
		case 0x01: // RLC C
			return void(this.c = this.rlc(this.c));
		case 0x02: // RLC D
			return void(this.d = this.rlc(this.d));
		case 0x03: // RLC E
			return void(this.e = this.rlc(this.e));
		case 0x04: // RLC H
			return void(this.h = this.rlc(this.h));
		case 0x05: // RLC L
			return void(this.l = this.rlc(this.l));
		case 0x06: // RLC (HL)
			return void this.write(v = this.l | this.h << 8, this.rlc(this.read(v)));
		case 0x07: // RLC A
			return void(this.a = this.rlc(this.a));
		case 0x08: // RRC B
			return void(this.b = this.rrc(this.b));
		case 0x09: // RRC C
			return void(this.c = this.rrc(this.c));
		case 0x0a: // RRC D
			return void(this.d = this.rrc(this.d));
		case 0x0b: // RRC E
			return void(this.e = this.rrc(this.e));
		case 0x0c: // RRC H
			return void(this.h = this.rrc(this.h));
		case 0x0d: // RRC L
			return void(this.l = this.rrc(this.l));
		case 0x0e: // RRC (HL)
			return void this.write(v = this.l | this.h << 8, this.rrc(this.read(v)));
		case 0x0f: // RRC A
			return void(this.a = this.rrc(this.a));
		case 0x10: // RL B
			return void(this.b = this.rl(this.b));
		case 0x11: // RL C
			return void(this.c = this.rl(this.c));
		case 0x12: // RL D
			return void(this.d = this.rl(this.d));
		case 0x13: // RL E
			return void(this.e = this.rl(this.e));
		case 0x14: // RL H
			return void(this.h = this.rl(this.h));
		case 0x15: // RL L
			return void(this.l = this.rl(this.l));
		case 0x16: // RL (HL)
			return void this.write(v = this.l | this.h << 8, this.rl(this.read(v)));
		case 0x17: // RL A
			return void(this.a = this.rl(this.a));
		case 0x18: // RR B
			return void(this.b = this.rr(this.b));
		case 0x19: // RR C
			return void(this.c = this.rr(this.c));
		case 0x1a: // RR D
			return void(this.d = this.rr(this.d));
		case 0x1b: // RR E
			return void(this.e = this.rr(this.e));
		case 0x1c: // RR H
			return void(this.h = this.rr(this.h));
		case 0x1d: // RR L
			return void(this.l = this.rr(this.l));
		case 0x1e: // RR (HL)
			return void this.write(v = this.l | this.h << 8, this.rr(this.read(v)));
		case 0x1f: // RR A
			return void(this.a = this.rr(this.a));
		case 0x20: // SLA B
			return void(this.b = this.sla(this.b));
		case 0x21: // SLA C
			return void(this.c = this.sla(this.c));
		case 0x22: // SLA D
			return void(this.d = this.sla(this.d));
		case 0x23: // SLA E
			return void(this.e = this.sla(this.e));
		case 0x24: // SLA H
			return void(this.h = this.sla(this.h));
		case 0x25: // SLA L
			return void(this.l = this.sla(this.l));
		case 0x26: // SLA (HL)
			return void this.write(v = this.l | this.h << 8, this.sla(this.read(v)));
		case 0x27: // SLA A
			return void(this.a = this.sla(this.a));
		case 0x28: // SRA B
			return void(this.b = this.sra(this.b));
		case 0x29: // SRA C
			return void(this.c = this.sra(this.c));
		case 0x2a: // SRA D
			return void(this.d = this.sra(this.d));
		case 0x2b: // SRA E
			return void(this.e = this.sra(this.e));
		case 0x2c: // SRA H
			return void(this.h = this.sra(this.h));
		case 0x2d: // SRA L
			return void(this.l = this.sra(this.l));
		case 0x2e: // SRA (HL)
			return void this.write(v = this.l | this.h << 8, this.sra(this.read(v)));
		case 0x2f: // SRA A
			return void(this.a = this.sra(this.a));
		case 0x38: // SRL B
			return void(this.b = this.srl(this.b));
		case 0x39: // SRL C
			return void(this.c = this.srl(this.c));
		case 0x3a: // SRL D
			return void(this.d = this.srl(this.d));
		case 0x3b: // SRL E
			return void(this.e = this.srl(this.e));
		case 0x3c: // SRL H
			return void(this.h = this.srl(this.h));
		case 0x3d: // SRL L
			return void(this.l = this.srl(this.l));
		case 0x3e: // SRL (HL)
			return void this.write(v = this.l | this.h << 8, this.srl(this.read(v)));
		case 0x3f: // SRL A
			return void(this.a = this.srl(this.a));
		case 0x40: // BIT 0,B
			return void this.bit(0, this.b);
		case 0x41: // BIT 0,C
			return void this.bit(0, this.c);
		case 0x42: // BIT 0,D
			return void this.bit(0, this.d);
		case 0x43: // BIT 0,E
			return void this.bit(0, this.e);
		case 0x44: // BIT 0,H
			return void this.bit(0, this.h);
		case 0x45: // BIT 0,L
			return void this.bit(0, this.l);
		case 0x46: // BIT 0,(HL)
			return void this.bit(0, this.read(this.l | this.h << 8));
		case 0x47: // BIT 0,A
			return void this.bit(0, this.a);
		case 0x48: // BIT 1,B
			return void this.bit(1, this.b);
		case 0x49: // BIT 1,C
			return void this.bit(1, this.c);
		case 0x4a: // BIT 1,D
			return void this.bit(1, this.d);
		case 0x4b: // BIT 1,E
			return void this.bit(1, this.e);
		case 0x4c: // BIT 1,H
			return void this.bit(1, this.h);
		case 0x4d: // BIT 1,L
			return void this.bit(1, this.l);
		case 0x4e: // BIT 1,(HL)
			return void this.bit(1, this.read(this.l | this.h << 8));
		case 0x4f: // BIT 1,A
			return void this.bit(1, this.a);
		case 0x50: // BIT 2,B
			return void this.bit(2, this.b);
		case 0x51: // BIT 2,C
			return void this.bit(2, this.c);
		case 0x52: // BIT 2,D
			return void this.bit(2, this.d);
		case 0x53: // BIT 2,E
			return void this.bit(2, this.e);
		case 0x54: // BIT 2,H
			return void this.bit(2, this.h);
		case 0x55: // BIT 2,L
			return void this.bit(2, this.l);
		case 0x56: // BIT 2,(HL)
			return void this.bit(2, this.read(this.l | this.h << 8));
		case 0x57: // BIT 2,A
			return void this.bit(2, this.a);
		case 0x58: // BIT 3,B
			return void this.bit(3, this.b);
		case 0x59: // BIT 3,C
			return void this.bit(3, this.c);
		case 0x5a: // BIT 3,D
			return void this.bit(3, this.d);
		case 0x5b: // BIT 3,E
			return void this.bit(3, this.e);
		case 0x5c: // BIT 3,H
			return void this.bit(3, this.h);
		case 0x5d: // BIT 3,L
			return void this.bit(3, this.l);
		case 0x5e: // BIT 3,(HL)
			return void this.bit(3, this.read(this.l | this.h << 8));
		case 0x5f: // BIT 3,A
			return void this.bit(3, this.a);
		case 0x60: // BIT 4,B
			return void this.bit(4, this.b);
		case 0x61: // BIT 4,C
			return void this.bit(4, this.c);
		case 0x62: // BIT 4,D
			return void this.bit(4, this.d);
		case 0x63: // BIT 4,E
			return void this.bit(4, this.e);
		case 0x64: // BIT 4,H
			return void this.bit(4, this.h);
		case 0x65: // BIT 4,L
			return void this.bit(4, this.l);
		case 0x66: // BIT 4,(HL)
			return void this.bit(4, this.read(this.l | this.h << 8));
		case 0x67: // BIT 4,A
			return void this.bit(4, this.a);
		case 0x68: // BIT 5,B
			return void this.bit(5, this.b);
		case 0x69: // BIT 5,C
			return void this.bit(5, this.c);
		case 0x6a: // BIT 5,D
			return void this.bit(5, this.d);
		case 0x6b: // BIT 5,E
			return void this.bit(5, this.e);
		case 0x6c: // BIT 5,H
			return void this.bit(5, this.h);
		case 0x6d: // BIT 5,L
			return void this.bit(5, this.l);
		case 0x6e: // BIT 5,(HL)
			return void this.bit(5, this.read(this.l | this.h << 8));
		case 0x6f: // BIT 5,A
			return void this.bit(5, this.a);
		case 0x70: // BIT 6,B
			return void this.bit(6, this.b);
		case 0x71: // BIT 6,C
			return void this.bit(6, this.c);
		case 0x72: // BIT 6,D
			return void this.bit(6, this.d);
		case 0x73: // BIT 6,E
			return void this.bit(6, this.e);
		case 0x74: // BIT 6,H
			return void this.bit(6, this.h);
		case 0x75: // BIT 6,L
			return void this.bit(6, this.l);
		case 0x76: // BIT 6,(HL)
			return void this.bit(6, this.read(this.l | this.h << 8));
		case 0x77: // BIT 6,A
			return void this.bit(6, this.a);
		case 0x78: // BIT 7,B
			return void this.bit(7, this.b);
		case 0x79: // BIT 7,C
			return void this.bit(7, this.c);
		case 0x7a: // BIT 7,D
			return void this.bit(7, this.d);
		case 0x7b: // BIT 7,E
			return void this.bit(7, this.e);
		case 0x7c: // BIT 7,H
			return void this.bit(7, this.h);
		case 0x7d: // BIT 7,L
			return void this.bit(7, this.l);
		case 0x7e: // BIT 7,(HL)
			return void this.bit(7, this.read(this.l | this.h << 8));
		case 0x7f: // BIT 7,A
			return void this.bit(7, this.a);
		case 0x80: // RES 0,B
			return void(this.b = this.constructor.res(0, this.b));
		case 0x81: // RES 0,C
			return void(this.c = this.constructor.res(0, this.c));
		case 0x82: // RES 0,D
			return void(this.d = this.constructor.res(0, this.d));
		case 0x83: // RES 0,E
			return void(this.e = this.constructor.res(0, this.e));
		case 0x84: // RES 0,H
			return void(this.h = this.constructor.res(0, this.h));
		case 0x85: // RES 0,L
			return void(this.l = this.constructor.res(0, this.l));
		case 0x86: // RES 0,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(0, this.read(v)));
		case 0x87: // RES 0,A
			return void(this.a = this.constructor.res(0, this.a));
		case 0x88: // RES 1,B
			return void(this.b = this.constructor.res(1, this.b));
		case 0x89: // RES 1,C
			return void(this.c = this.constructor.res(1, this.c));
		case 0x8a: // RES 1,D
			return void(this.d = this.constructor.res(1, this.d));
		case 0x8b: // RES 1,E
			return void(this.e = this.constructor.res(1, this.e));
		case 0x8c: // RES 1,H
			return void(this.h = this.constructor.res(1, this.h));
		case 0x8d: // RES 1,L
			return void(this.l = this.constructor.res(1, this.l));
		case 0x8e: // RES 1,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(1, this.read(v)));
		case 0x8f: // RES 1,A
			return void(this.a = this.constructor.res(1, this.a));
		case 0x90: // RES 2,B
			return void(this.b = this.constructor.res(2, this.b));
		case 0x91: // RES 2,C
			return void(this.c = this.constructor.res(2, this.c));
		case 0x92: // RES 2,D
			return void(this.d = this.constructor.res(2, this.d));
		case 0x93: // RES 2,E
			return void(this.e = this.constructor.res(2, this.e));
		case 0x94: // RES 2,H
			return void(this.h = this.constructor.res(2, this.h));
		case 0x95: // RES 2,L
			return void(this.l = this.constructor.res(2, this.l));
		case 0x96: // RES 2,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(2, this.read(v)));
		case 0x97: // RES 2,A
			return void(this.a = this.constructor.res(2, this.a));
		case 0x98: // RES 3,B
			return void(this.b = this.constructor.res(3, this.b));
		case 0x99: // RES 3,C
			return void(this.c = this.constructor.res(3, this.c));
		case 0x9a: // RES 3,D
			return void(this.d = this.constructor.res(3, this.d));
		case 0x9b: // RES 3,E
			return void(this.e = this.constructor.res(3, this.e));
		case 0x9c: // RES 3,H
			return void(this.h = this.constructor.res(3, this.h));
		case 0x9d: // RES 3,L
			return void(this.l = this.constructor.res(3, this.l));
		case 0x9e: // RES 3,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(3, this.read(v)));
		case 0x9f: // RES 3,A
			return void(this.a = this.constructor.res(3, this.a));
		case 0xa0: // RES 4,B
			return void(this.b = this.constructor.res(4, this.b));
		case 0xa1: // RES 4,C
			return void(this.c = this.constructor.res(4, this.c));
		case 0xa2: // RES 4,D
			return void(this.d = this.constructor.res(4, this.d));
		case 0xa3: // RES 4,E
			return void(this.e = this.constructor.res(4, this.e));
		case 0xa4: // RES 4,H
			return void(this.h = this.constructor.res(4, this.h));
		case 0xa5: // RES 4,L
			return void(this.l = this.constructor.res(4, this.l));
		case 0xa6: // RES 4,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(4, this.read(v)));
		case 0xa7: // RES 4,A
			return void(this.a = this.constructor.res(4, this.a));
		case 0xa8: // RES 5,B
			return void(this.b = this.constructor.res(5, this.b));
		case 0xa9: // RES 5,C
			return void(this.c = this.constructor.res(5, this.c));
		case 0xaa: // RES 5,D
			return void(this.d = this.constructor.res(5, this.d));
		case 0xab: // RES 5,E
			return void(this.e = this.constructor.res(5, this.e));
		case 0xac: // RES 5,H
			return void(this.h = this.constructor.res(5, this.h));
		case 0xad: // RES 5,L
			return void(this.l = this.constructor.res(5, this.l));
		case 0xae: // RES 5,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(5, this.read(v)));
		case 0xaf: // RES 5,A
			return void(this.a = this.constructor.res(5, this.a));
		case 0xb0: // RES 6,B
			return void(this.b = this.constructor.res(6, this.b));
		case 0xb1: // RES 6,C
			return void(this.c = this.constructor.res(6, this.c));
		case 0xb2: // RES 6,D
			return void(this.d = this.constructor.res(6, this.d));
		case 0xb3: // RES 6,E
			return void(this.e = this.constructor.res(6, this.e));
		case 0xb4: // RES 6,H
			return void(this.h = this.constructor.res(6, this.h));
		case 0xb5: // RES 6,L
			return void(this.l = this.constructor.res(6, this.l));
		case 0xb6: // RES 6,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(6, this.read(v)));
		case 0xb7: // RES 6,A
			return void(this.a = this.constructor.res(6, this.a));
		case 0xb8: // RES 7,B
			return void(this.b = this.constructor.res(7, this.b));
		case 0xb9: // RES 7,C
			return void(this.c = this.constructor.res(7, this.c));
		case 0xba: // RES 7,D
			return void(this.d = this.constructor.res(7, this.d));
		case 0xbb: // RES 7,E
			return void(this.e = this.constructor.res(7, this.e));
		case 0xbc: // RES 7,H
			return void(this.h = this.constructor.res(7, this.h));
		case 0xbd: // RES 7,L
			return void(this.l = this.constructor.res(7, this.l));
		case 0xbe: // RES 7,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.res(7, this.read(v)));
		case 0xbf: // RES 7,A
			return void(this.a = this.constructor.res(7, this.a));
		case 0xc0: // SET 0,B
			return void(this.b = this.constructor.set(0, this.b));
		case 0xc1: // SET 0,C
			return void(this.c = this.constructor.set(0, this.c));
		case 0xc2: // SET 0,D
			return void(this.d = this.constructor.set(0, this.d));
		case 0xc3: // SET 0,E
			return void(this.e = this.constructor.set(0, this.e));
		case 0xc4: // SET 0,H
			return void(this.h = this.constructor.set(0, this.h));
		case 0xc5: // SET 0,L
			return void(this.l = this.constructor.set(0, this.l));
		case 0xc6: // SET 0,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(0, this.read(v)));
		case 0xc7: // SET 0,A
			return void(this.a = this.constructor.set(0, this.a));
		case 0xc8: // SET 1,B
			return void(this.b = this.constructor.set(1, this.b));
		case 0xc9: // SET 1,C
			return void(this.c = this.constructor.set(1, this.c));
		case 0xca: // SET 1,D
			return void(this.d = this.constructor.set(1, this.d));
		case 0xcb: // SET 1,E
			return void(this.e = this.constructor.set(1, this.e));
		case 0xcc: // SET 1,H
			return void(this.h = this.constructor.set(1, this.h));
		case 0xcd: // SET 1,L
			return void(this.l = this.constructor.set(1, this.l));
		case 0xce: // SET 1,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(1, this.read(v)));
		case 0xcf: // SET 1,A
			return void(this.a = this.constructor.set(1, this.a));
		case 0xd0: // SET 2,B
			return void(this.b = this.constructor.set(2, this.b));
		case 0xd1: // SET 2,C
			return void(this.c = this.constructor.set(2, this.c));
		case 0xd2: // SET 2,D
			return void(this.d = this.constructor.set(2, this.d));
		case 0xd3: // SET 2,E
			return void(this.e = this.constructor.set(2, this.e));
		case 0xd4: // SET 2,H
			return void(this.h = this.constructor.set(2, this.h));
		case 0xd5: // SET 2,L
			return void(this.l = this.constructor.set(2, this.l));
		case 0xd6: // SET 2,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(2, this.read(v)));
		case 0xd7: // SET 2,A
			return void(this.a = this.constructor.set(2, this.a));
		case 0xd8: // SET 3,B
			return void(this.b = this.constructor.set(3, this.b));
		case 0xd9: // SET 3,C
			return void(this.c = this.constructor.set(3, this.c));
		case 0xda: // SET 3,D
			return void(this.d = this.constructor.set(3, this.d));
		case 0xdb: // SET 3,E
			return void(this.e = this.constructor.set(3, this.e));
		case 0xdc: // SET 3,H
			return void(this.h = this.constructor.set(3, this.h));
		case 0xdd: // SET 3,L
			return void(this.l = this.constructor.set(3, this.l));
		case 0xde: // SET 3,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(3, this.read(v)));
		case 0xdf: // SET 3,A
			return void(this.a = this.constructor.set(3, this.a));
		case 0xe0: // SET 4,B
			return void(this.b = this.constructor.set(4, this.b));
		case 0xe1: // SET 4,C
			return void(this.c = this.constructor.set(4, this.c));
		case 0xe2: // SET 4,D
			return void(this.d = this.constructor.set(4, this.d));
		case 0xe3: // SET 4,E
			return void(this.e = this.constructor.set(4, this.e));
		case 0xe4: // SET 4,H
			return void(this.h = this.constructor.set(4, this.h));
		case 0xe5: // SET 4,L
			return void(this.l = this.constructor.set(4, this.l));
		case 0xe6: // SET 4,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(4, this.read(v)));
		case 0xe7: // SET 4,A
			return void(this.a = this.constructor.set(4, this.a));
		case 0xe8: // SET 5,B
			return void(this.b = this.constructor.set(5, this.b));
		case 0xe9: // SET 5,C
			return void(this.c = this.constructor.set(5, this.c));
		case 0xea: // SET 5,D
			return void(this.d = this.constructor.set(5, this.d));
		case 0xeb: // SET 5,E
			return void(this.e = this.constructor.set(5, this.e));
		case 0xec: // SET 5,H
			return void(this.h = this.constructor.set(5, this.h));
		case 0xed: // SET 5,L
			return void(this.l = this.constructor.set(5, this.l));
		case 0xee: // SET 5,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(5, this.read(v)));
		case 0xef: // SET 5,A
			return void(this.a = this.constructor.set(5, this.a));
		case 0xf0: // SET 6,B
			return void(this.b = this.constructor.set(6, this.b));
		case 0xf1: // SET 6,C
			return void(this.c = this.constructor.set(6, this.c));
		case 0xf2: // SET 6,D
			return void(this.d = this.constructor.set(6, this.d));
		case 0xf3: // SET 6,E
			return void(this.e = this.constructor.set(6, this.e));
		case 0xf4: // SET 6,H
			return void(this.h = this.constructor.set(6, this.h));
		case 0xf5: // SET 6,L
			return void(this.l = this.constructor.set(6, this.l));
		case 0xf6: // SET 6,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(6, this.read(v)));
		case 0xf7: // SET 6,A
			return void(this.a = this.constructor.set(6, this.a));
		case 0xf8: // SET 7,B
			return void(this.b = this.constructor.set(7, this.b));
		case 0xf9: // SET 7,C
			return void(this.c = this.constructor.set(7, this.c));
		case 0xfa: // SET 7,D
			return void(this.d = this.constructor.set(7, this.d));
		case 0xfb: // SET 7,E
			return void(this.e = this.constructor.set(7, this.e));
		case 0xfc: // SET 7,H
			return void(this.h = this.constructor.set(7, this.h));
		case 0xfd: // SET 7,L
			return void(this.l = this.constructor.set(7, this.l));
		case 0xfe: // SET 7,(HL)
			return void this.write(v = this.l | this.h << 8, this.constructor.set(7, this.read(v)));
		case 0xff: // SET 7,A
			return void(this.a = this.constructor.set(7, this.a));
		default:
			this.undefsize = 2;
			if (this.undef)
				this.undef(this.arg);
			return;
		}
	}

	execute_dd() {
		let v;

		switch (this.fetch()) {
		case 0x09: // ADD IX,BC
			this.ixl = (v = Z80.aAdd[0][this.c][this.ixl]) & 0xff;
			this.ixh = (v = Z80.aAdd[v >>> 8 & 1][this.b][this.ixh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x19: // ADD IX,DE
			this.ixl = (v = Z80.aAdd[0][this.e][this.ixl]) & 0xff;
			this.ixh = (v = Z80.aAdd[v >>> 8 & 1][this.d][this.ixh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x21: // LD IX,nn
			return void([this.ixl, this.ixh] = [this.fetch(), this.fetch()]);
		case 0x22: // LD (nn),IX
			this.write(v = this.fetch() | this.fetch() << 8, this.ixl);
			this.write1(v, this.ixh);
			return;
		case 0x23: // INC IX
			this.ixl = (v = (this.ixl | this.ixh << 8) + 1 & 0xffff) & 0xff;
			this.ixh = v >>> 8;
			return;
		case 0x24: // INC IXH (undefined operation)
			return void(this.ixh = this.inc(this.ixh));
		case 0x25: // DEC IXH (undefined operation)
			return void(this.ixh = this.dec(this.ixh));
		case 0x26: // LD IXH,n (undefined operation)
			return void(this.ixh = this.fetch());
		case 0x29: // ADD IX,IX
			this.ixl = (v = Z80.aAdd[0][this.ixl][this.ixl]) & 0xff;
			this.ixh = (v = Z80.aAdd[v >>> 8 & 1][this.ixh][this.ixh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x2a: // LD IX,(nn)
			this.ixl = this.read(v = this.fetch() | this.fetch() << 8);
			this.ixh = this.read1(v);
			return;
		case 0x2b: // DEC IX
			this.ixl = (v = (this.ixl | this.ixh << 8) - 1 & 0xffff) & 0xff;
			this.ixh = v >>> 8;
			return;
		case 0x2c: // INC IXL (undefined operation)
			return void(this.ixl = this.inc(this.ixl));
		case 0x2d: // DEC IXL (undefined operation)
			return void(this.ixl = this.dec(this.ixl));
		case 0x2e: // LD IXL,n (undefined operation)
			return void(this.ixl = this.fetch());
		case 0x34: // INC (IX+d)
			return void this.write(v = this.disp(this.ixh, this.ixl), this.inc(this.read(v)));
		case 0x35: // DEC (IX+d)
			return void this.write(v = this.disp(this.ixh, this.ixl), this.dec(this.read(v)));
		case 0x36: // LD (IX+d),n
			return void this.write(this.disp(this.ixh, this.ixl), this.fetch());
		case 0x39: // ADD IX,SP
			this.ixl = (v = Z80.aAdd[0][this.sp & 0xff][this.ixl]) & 0xff;
			this.ixh = (v = Z80.aAdd[v >>> 8 & 1][this.sp >>> 8][this.ixh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x44: // LD B,IXH (undefined operation)
			return void(this.b = this.ixh);
		case 0x45: // LD B,IXL (undefined operation)
			return void(this.b = this.ixl);
		case 0x46: // LD B,(IX+d)
			return void(this.b = this.read(this.disp(this.ixh, this.ixl)));
		case 0x4c: // LD C,IXH (undefined operation)
			return void(this.c = this.ixh);
		case 0x4d: // LD C,IXL (undefined operation)
			return void(this.c = this.ixl);
		case 0x4e: // LD C,(IX+d)
			return void(this.c = this.read(this.disp(this.ixh, this.ixl)));
		case 0x54: // LD D,IXH (undefined operation)
			return void(this.d = this.ixh);
		case 0x55: // LD D,IXL (undefined operation)
			return void(this.d = this.ixl);
		case 0x56: // LD D,(IX+d)
			return void(this.d = this.read(this.disp(this.ixh, this.ixl)));
		case 0x5c: // LD E,IXH (undefined operation)
			return void(this.e = this.ixh);
		case 0x5d: // LD E,IXL (undefined operation)
			return void(this.e = this.ixl);
		case 0x5e: // LD E,(IX+d)
			return void(this.e = this.read(this.disp(this.ixh, this.ixl)));
		case 0x60: // LD IXH,B (undefined operation)
			return void(this.ixh = this.b);
		case 0x61: // LD IXH,C (undefined operation)
			return void(this.ixh = this.c);
		case 0x62: // LD IXH,D (undefined operation)
			return void(this.ixh = this.d);
		case 0x63: // LD IXH,E (undefined operation)
			return void(this.ixh = this.e);
		case 0x66: // LD H,(IX+d)
			return void(this.h = this.read(this.disp(this.ixh, this.ixl)));
		case 0x67: // LD IXH,A (undefined operation)
			return void(this.ixh = this.a);
		case 0x68: // LD IXL,B (undefined operation)
			return void(this.ixl = this.b);
		case 0x69: // LD IXL,C (undefined operation)
			return void(this.ixl = this.c);
		case 0x6a: // LD IXL,D (undefined operation)
			return void(this.ixl = this.d);
		case 0x6b: // LD IXL,E (undefined operation)
			return void(this.ixl = this.e);
		case 0x6e: // LD L,(IX+d)
			return void(this.l = this.read(this.disp(this.ixh, this.ixl)));
		case 0x6f: // LD IXL,A (undefined operation)
			return void(this.ixl = this.a);
		case 0x70: // LD (IX+d),B
			return void this.write(this.disp(this.ixh, this.ixl), this.b);
		case 0x71: // LD (IX+d),C
			return void this.write(this.disp(this.ixh, this.ixl), this.c);
		case 0x72: // LD (IX+d),D
			return void this.write(this.disp(this.ixh, this.ixl), this.d);
		case 0x73: // LD (IX+d),E
			return void this.write(this.disp(this.ixh, this.ixl), this.e);
		case 0x74: // LD (IX+d),H
			return void this.write(this.disp(this.ixh, this.ixl), this.h);
		case 0x75: // LD (IX+d),L
			return void this.write(this.disp(this.ixh, this.ixl), this.l);
		case 0x77: // LD (IX+d),A
			return void this.write(this.disp(this.ixh, this.ixl), this.a);
		case 0x7c: // LD A,IXH (undefined operation)
			return void(this.a = this.ixh);
		case 0x7d: // LD A,IXL (undefined operation)
			return void(this.a = this.ixl);
		case 0x7e: // LD A,(IX+d)
			return void(this.a = this.read(this.disp(this.ixh, this.ixl)));
		case 0x84: // ADD A,IXH (undefined operation)
			return void this.add(this.ixh);
		case 0x85: // ADD A,IXL (undefined operation)
			return void this.add(this.ixl);
		case 0x86: // ADD A,(IX+d)
			return void this.add(this.read(this.disp(this.ixh, this.ixl)));
		case 0x8c: // ADC A,IXH (undefined operation)
			return void this.adc(this.ixh);
		case 0x8d: // ADC A,IXL (undefined operation)
			return void this.adc(this.ixl);
		case 0x8e: // ADC A,(IX+d)
			return void this.adc(this.read(this.disp(this.ixh, this.ixl)));
		case 0x94: // SUB IXH (undefined operation)
			return void this.sub(this.ixh);
		case 0x95: // SUB IXL (undefined operation)
			return void this.sub(this.ixl);
		case 0x96: // SUB (IX+d)
			return void this.sub(this.read(this.disp(this.ixh, this.ixl)));
		case 0x9c: // SBC A,IXH (undefined operation)
			return void this.sbc(this.ixh);
		case 0x9d: // SBC A,IXL (undefined operation)
			return void this.sbc(this.ixl);
		case 0x9e: // SBC A,(IX+d)
			return void this.sbc(this.read(this.disp(this.ixh, this.ixl)));
		case 0xa4: // AND IXH (undefined operation)
			return void this.and(this.ixh);
		case 0xa5: // AND IXL (undefined operation)
			return void this.and(this.ixl);
		case 0xa6: // AND (IX+d)
			return void this.and(this.read(this.disp(this.ixh, this.ixl)));
		case 0xac: // XOR IXH (undefined operation)
			return void this.xor(this.ixh);
		case 0xad: // XOR IXL (undefined operation)
			return void this.xor(this.ixl);
		case 0xae: // XOR (IX+d)
			return void this.xor(this.read(this.disp(this.ixh, this.ixl)));
		case 0xb4: // OR IXH (undefined operation)
			return void this.or(this.ixh);
		case 0xb5: // OR IXL (undefined operation)
			return void this.or(this.ixl);
		case 0xb6: // OR (IX+d)
			return void this.or(this.read(this.disp(this.ixh, this.ixl)));
		case 0xbc: // CP IXH (undefined operation)
			return void this.cp(this.ixh);
		case 0xbd: // CP IXL (undefined operation)
			return void this.cp(this.ixl);
		case 0xbe: // CP (IX+d)
			return void this.cp(this.read(this.disp(this.ixh, this.ixl)));
		case 0xcb:
			v = this.disp(this.ixh, this.ixl);
			switch (this.fetch()) {
			case 0x06: // RLC (IX+d)
				return void this.write(v, this.rlc(this.read(v)));
			case 0x0e: // RRC (IX+d)
				return void this.write(v, this.rrc(this.read(v)));
			case 0x16: // RL (IX+d)
				return void this.write(v, this.rl(this.read(v)));
			case 0x1e: // RR (IX+d)
				return void this.write(v, this.rr(this.read(v)));
			case 0x26: // SLA (IX+d)
				return void this.write(v, this.sla(this.read(v)));
			case 0x2e: // SRA (IX+d)
				return void this.write(v, this.sra(this.read(v)));
			case 0x3e: // SRL (IX+d)
				return void this.write(v, this.srl(this.read(v)));
			case 0x46: // BIT 0,(IX+d)
				return void this.bit(0, this.read(v));
			case 0x4e: // BIT 1,(IX+d)
				return void this.bit(1, this.read(v));
			case 0x56: // BIT 2,(IX+d)
				return void this.bit(2, this.read(v));
			case 0x5e: // BIT 3,(IX+d)
				return void this.bit(3, this.read(v));
			case 0x66: // BIT 4,(IX+d)
				return void this.bit(4, this.read(v));
			case 0x6e: // BIT 5,(IX+d)
				return void this.bit(5, this.read(v));
			case 0x76: // BIT 6,(IX+d)
				return void this.bit(6, this.read(v));
			case 0x7e: // BIT 7,(IX+d)
				return void this.bit(7, this.read(v));
			case 0x86: // RES 0,(IX+d)
				return void this.write(v, this.constructor.res(0, this.read(v)));
			case 0x8e: // RES 1,(IX+d)
				return void this.write(v, this.constructor.res(1, this.read(v)));
			case 0x96: // RES 2,(IX+d)
				return void this.write(v, this.constructor.res(2, this.read(v)));
			case 0x9e: // RES 3,(IX+d)
				return void this.write(v, this.constructor.res(3, this.read(v)));
			case 0xa6: // RES 4,(IX+d)
				return void this.write(v, this.constructor.res(4, this.read(v)));
			case 0xae: // RES 5,(IX+d)
				return void this.write(v, this.constructor.res(5, this.read(v)));
			case 0xb6: // RES 6,(IX+d)
				return void this.write(v, this.constructor.res(6, this.read(v)));
			case 0xbe: // RES 7,(IX+d)
				return void this.write(v, this.constructor.res(7, this.read(v)));
			case 0xc6: // SET 0,(IX+d)
				return void this.write(v, this.constructor.set(0, this.read(v)));
			case 0xce: // SET 1,(IX+d)
				return void this.write(v, this.constructor.set(1, this.read(v)));
			case 0xd6: // SET 2,(IX+d)
				return void this.write(v, this.constructor.set(2, this.read(v)));
			case 0xde: // SET 3,(IX+d)
				return void this.write(v, this.constructor.set(3, this.read(v)));
			case 0xe6: // SET 4,(IX+d)
				return void this.write(v, this.constructor.set(4, this.read(v)));
			case 0xee: // SET 5,(IX+d)
				return void this.write(v, this.constructor.set(5, this.read(v)));
			case 0xf6: // SET 6,(IX+d)
				return void this.write(v, this.constructor.set(6, this.read(v)));
			case 0xfe: // SET 7,(IX+d)
				return void this.write(v, this.constructor.set(7, this.read(v)));
			default:
				this.undefsize = 4;
				if (this.undef)
					this.undef(this.arg);
				return;
			}
		case 0xe1: // POP IX
			return void([this.ixl, this.ixh] = [this.pop(), this.pop()]);
		case 0xe3: // EX (SP),IX
			v = this.read(this.sp);
			this.write(this.sp, this.ixl);
			this.ixl = v;
			v = this.read1(this.sp);
			this.write1(this.sp, this.ixh);
			this.ixh = v;
			return;
		case 0xe5: // PUSH IX
			this.push(this.ixh);
			this.push(this.ixl);
			return;
		case 0xe9: // JP (IX)
			return void(this.pc = this.ixl | this.ixh << 8);
		case 0xf9: // LD SP,IX
			return void(this.sp = this.ixl | this.ixh << 8);
		default:
			this.undefsize = 2;
			if (this.undef)
				this.undef(this.arg);
			return;
		}
	}

	execute_ed() {
		let v;

		switch (this.fetch()) {
		case 0x40: // IN B,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.b = this.ioread(this.b, this.c)]);
		case 0x41: // OUT (C),B
			return void this.iowrite(this.b, this.c, this.b);
		case 0x42: // SBC HL,BC
			this.l = (v = Z80.aSub[this.f & 1][this.c][this.l]) & 0xff;
			this.h = (v = Z80.aSub[v >>> 8 & 1][this.b][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x43: // LD (nn),BC
			this.write(v = this.fetch() | this.fetch() << 8, this.c);
			this.write1(v, this.b);
			return;
		case 0x44: // NEG
			this.a = (v = Z80.aSub[0][this.a][0]) & 0xff;
			this.f = v >>> 8;
			return;
		case 0x45: // RETN
			this.iff = (this.iff & 2) !== 0 ? 3 : 0;
			this.ret(true);
			return;
		case 0x46: // IM 0
			return void(this.im = 0);
		case 0x47: // LD I,A
			return void(this.i = this.a);
		case 0x48: // IN C,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.c = this.ioread(this.b, this.c)]);
		case 0x49: // OUT (C),C
			return void this.iowrite(this.b, this.c, this.c);
		case 0x4a: // ADC HL,BC
			this.l = (v = Z80.aAdd[this.f & 1][this.c][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.b][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x4b: // LD BC,(nn)
			this.c = this.read(v = this.fetch() | this.fetch() << 8);
			this.b = this.read1(v);
			return;
		case 0x4d: // RETI
			return void this.ret(true);
		case 0x4f: // LD R,A
			return void(this.r = this.a & 0x7f);
		case 0x50: // IN D,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.d = this.ioread(this.b, this.c)]);
		case 0x51: // OUT (C),D
			return void this.iowrite(this.b, this.c, this.d);
		case 0x52: // SBC HL,DE
			this.l = (v = Z80.aSub[this.f & 1][this.e][this.l]) & 0xff;
			this.h = (v = Z80.aSub[v >>> 8 & 1][this.d][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x53: // LD (nn),DE
			this.write(v = this.fetch() | this.fetch() << 8, this.e);
			this.write1(v, this.d);
			return;
		case 0x56: // IM 1
			return void(this.im = 1);
		case 0x57: // LD A,I
			return void(this.f = this.f & 1 | Z80.fLogic[this.a = this.i] & 0xc0 | this.iff << 1 & 4);
		case 0x58: // IN E,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.e = this.ioread(this.b, this.c)]);
		case 0x59: // OUT (C),E
			return void this.iowrite(this.b, this.c, this.e);
		case 0x5a: // ADC HL,DE
			this.l = (v = Z80.aAdd[this.f & 1][this.e][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.d][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x5b: // LD DE,(nn)
			this.e = this.read(v = this.fetch() | this.fetch() << 8);
			this.d = this.read1(v);
			return;
		case 0x5e: // IM 2
			return void(this.im = 2);
		case 0x5f: // LD A,R
			return void(this.f = this.f & 1 | Z80.fLogic[this.a = this.r] & 0xc0 | this.iff << 1 & 4);
		case 0x60: // IN H,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.h = this.ioread(this.b, this.c)]);
		case 0x61: // OUT (C),H
			return void this.iowrite(this.b, this.c, this.h);
		case 0x62: // SBC HL,HL
			this.l = (v = Z80.aSub[this.f & 1][this.l][this.l]) & 0xff;
			this.h = (v = Z80.aSub[v >>> 8 & 1][this.h][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x67: // RRD
			this.a = this.a & 0xf0 | (v = this.read(this.l | this.h << 8) | this.a << 8) & 0x0f;
			this.f = this.f & 1 | Z80.fLogic[this.write(this.l | this.h << 8, v >>> 4 & 0xff)] & 0xc4;
			return;
		case 0x68: // IN L,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.l = this.ioread(this.b, this.c)]);
		case 0x69: // OUT (C),L
			return void this.iowrite(this.b, this.c, this.l);
		case 0x6a: // ADC HL,HL
			this.l = (v = Z80.aAdd[this.f & 1][this.l][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.h][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x6f: // RLD
			this.a = this.a & 0xf0 | (v = this.read(this.l | this.h << 8) << 4 | this.a & 0x0f) >>> 8;
			this.f = this.f & 1 | Z80.fLogic[this.write(this.l | this.h << 8, v & 0xff)] & 0xc4;
			return;
		case 0x72: // SBC HL,SP
			this.l = (v = Z80.aSub[this.f & 1][this.sp & 0xff][this.l]) & 0xff;
			this.h = (v = Z80.aSub[v >>> 8 & 1][this.sp >>> 8][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x73: // LD (nn),SP
			this.write(v = this.fetch() | this.fetch() << 8, this.sp & 0xff);
			this.write1(v, this.sp >>> 8);
			return;
		case 0x78: // IN A,(C)
			return void(this.f = this.f & 1 | Z80.fLogic[this.a = this.ioread(this.b, this.c)]);
		case 0x79: // OUT (C),A
			return void this.iowrite(this.b, this.c, this.a);
		case 0x7a: // ADC HL,SP
			this.l = (v = Z80.aAdd[this.f & 1][this.sp & 0xff][this.l]) & 0xff;
			this.h = (v = Z80.aAdd[v >>> 8 & 1][this.sp >>> 8][this.h]) & 0xff;
			this.f = v >>> 8 & ~((this.l !== 0) << 6);
			return;
		case 0x7b: // LD SP,(nn)
			return void(this.sp = this.read(v = this.fetch() | this.fetch() << 8) | this.read1(v) << 8);
		case 0xa0: // LDI
			return void this.ldi();
		case 0xa1: // CPI
			return void this.cpi();
		case 0xa2: // INI
			return void this.ini();
		case 0xa3: // OUTI
			return void this.outi();
		case 0xa8: // LDD
			return void this.ldd();
		case 0xa9: // CPD
			return void this.cpd();
		case 0xaa: // IND
			return void this.ind();
		case 0xab: // OUTD
			return void this.outd();
		case 0xb0: // LDIR
			this.ldi();
			if ((this.f & 4) !== 0)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xb1: // CPIR
			this.cpi();
			if ((this.f & 0x44) === 4)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xb2: // INIR
			this.ini();
			if ((this.f & 0x40) === 0)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xb3: // OTIR
			this.outi();
			if ((this.f & 0x40) === 0)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xb8: // LDDR
			this.ldd();
			if ((this.f & 4) !== 0)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xb9: // CPDR
			this.cpd();
			if ((this.f & 0x44) === 4)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xba: // INDR
			this.ind();
			if ((this.f & 0x40) === 0)
				this.pc = this.pc - 2 & 0xffff;
			return;
		case 0xbb: // OTDR
			this.outd();
			if ((this.f & 0x40) === 0)
				this.pc = this.pc - 2 & 0xffff;
			return;
		default:
			this.undefsize = 2;
			if (this.undef)
				this.undef(this.arg);
			return;
		}
	}

	execute_fd() {
		let v;

		switch (this.fetch()) {
		case 0x09: // ADD IY,BC
			this.iyl = (v = Z80.aAdd[0][this.c][this.iyl]) & 0xff;
			this.iyh = (v = Z80.aAdd[v >>> 8 & 1][this.b][this.iyh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x19: // ADD IY,DE
			this.iyl = (v = Z80.aAdd[0][this.e][this.iyl]) & 0xff;
			this.iyh = (v = Z80.aAdd[v >>> 8 & 1][this.d][this.iyh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x21: // LD IY,nn
			return void([this.iyl, this.iyh] = [this.fetch(), this.fetch()]);
		case 0x22: // LD (nn),IY
			this.write(v = this.fetch() | this.fetch() << 8, this.iyl);
			this.write1(v, this.iyh);
			return;
		case 0x23: // INC IY
			this.iyl = (v = (this.iyl | this.iyh << 8) + 1 & 0xffff) & 0xff;
			this.iyh = v >>> 8;
			return;
		case 0x24: // INC IYH (undefined operation)
			return void(this.iyh = this.inc(this.iyh));
		case 0x25: // DEC IYH (undefined operation)
			return void(this.iyh = this.dec(this.iyh));
		case 0x26: // LD IYH,n (undefined operation)
			return void(this.iyh = this.fetch());
		case 0x29: // ADD IY,IY
			this.iyl = (v = Z80.aAdd[0][this.iyl][this.iyl]) & 0xff;
			this.iyh = (v = Z80.aAdd[v >>> 8 & 1][this.iyh][this.iyh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x2a: // LD IY,(nn)
			this.iyl = this.read(v = this.fetch() | this.fetch() << 8);
			this.iyh = this.read1(v);
			return;
		case 0x2b: // DEC IY
			this.iyl = (v = (this.iyl | this.iyh << 8) - 1 & 0xffff) & 0xff;
			this.iyh = v >>> 8;
			return;
		case 0x2c: // INC IYL (undefined operation)
			return void(this.iyl = this.inc(this.iyl));
		case 0x2d: // DEC IYL (undefined operation)
			return void(this.iyl = this.dec(this.iyl));
		case 0x2e: // LD IYL,n (undefined operation)
			return void(this.iyl = this.fetch());
		case 0x34: // INC (IY+d)
			return void this.write(v = this.disp(this.iyh, this.iyl), this.inc(this.read(v)));
		case 0x35: // DEC (IY+d)
			return void this.write(v = this.disp(this.iyh, this.iyl), this.dec(this.read(v)));
		case 0x36: // LD (IY+d),n
			return void this.write(this.disp(this.iyh, this.iyl), this.fetch());
		case 0x39: // ADD IY,SP
			this.iyl = (v = Z80.aAdd[0][this.sp & 0xff][this.iyl]) & 0xff;
			this.iyh = (v = Z80.aAdd[v >>> 8 & 1][this.sp >>> 8][this.iyh]) & 0xff;
			this.f = this.f & 0xc4 | v >>> 8 & 1;
			return;
		case 0x44: // LD B,IYH (undefined operation)
			return void(this.b = this.iyh);
		case 0x45: // LD B,IYL (undefined operation)
			return void(this.b = this.iyl);
		case 0x46: // LD B,(IY+d)
			return void(this.b = this.read(this.disp(this.iyh, this.iyl)));
		case 0x4c: // LD C,IYH (undefined operation)
			return void(this.c = this.iyh);
		case 0x4d: // LD C,IYL (undefined operation)
			return void(this.c = this.iyl);
		case 0x4e: // LD C,(IY+d)
			return void(this.c = this.read(this.disp(this.iyh, this.iyl)));
		case 0x54: // LD D,IYH (undefined operation)
			return void(this.d = this.iyh);
		case 0x55: // LD D,IYL (undefined operation)
			return void(this.d = this.iyl);
		case 0x56: // LD D,(IY+d)
			return void(this.d = this.read(this.disp(this.iyh, this.iyl)));
		case 0x5c: // LD E,IYH (undefined operation)
			return void(this.e = this.iyh);
		case 0x5d: // LD E,IYL (undefined operation)
			return void(this.e = this.iyl);
		case 0x5e: // LD E,(IY+d)
			return void(this.e = this.read(this.disp(this.iyh, this.iyl)));
		case 0x60: // LD IYH,B (undefined operation)
			return void(this.iyh = this.b);
		case 0x61: // LD IYH,C (undefined operation)
			return void(this.iyh = this.c);
		case 0x62: // LD IYH,D (undefined operation)
			return void(this.iyh = this.d);
		case 0x63: // LD IYH,E (undefined operation)
			return void(this.iyh = this.e);
		case 0x66: // LD H,(IY+d)
			return void(this.h = this.read(this.disp(this.iyh, this.iyl)));
		case 0x67: // LD IYH,A (undefined operation)
			return void(this.iyh = this.a);
		case 0x68: // LD IYL,B (undefined operation)
			return void(this.iyl = this.b);
		case 0x69: // LD IYL,C (undefined operation)
			return void(this.iyl = this.c);
		case 0x6a: // LD IYL,D (undefined operation)
			return void(this.iyl = this.d);
		case 0x6b: // LD IYL,E (undefined operation)
			return void(this.iyl = this.e);
		case 0x6e: // LD L,(IY+d)
			return void(this.l = this.read(this.disp(this.iyh, this.iyl)));
		case 0x6f: // LD IYL,A (undefined operation)
			return void(this.iyl = this.a);
		case 0x70: // LD (IY+d),B
			return void this.write(this.disp(this.iyh, this.iyl), this.b);
		case 0x71: // LD (IY+d),C
			return void this.write(this.disp(this.iyh, this.iyl), this.c);
		case 0x72: // LD (IY+d),D
			return void this.write(this.disp(this.iyh, this.iyl), this.d);
		case 0x73: // LD (IY+d),E
			return void this.write(this.disp(this.iyh, this.iyl), this.e);
		case 0x74: // LD (IY+d),H
			return void this.write(this.disp(this.iyh, this.iyl), this.h);
		case 0x75: // LD (IY+d),L
			return void this.write(this.disp(this.iyh, this.iyl), this.l);
		case 0x77: // LD (IY+d),A
			return void this.write(this.disp(this.iyh, this.iyl), this.a);
		case 0x7c: // LD A,IYH (undefined operation)
			return void(this.a = this.iyh);
		case 0x7d: // LD A,IYL (undefined operation)
			return void(this.a = this.iyl);
		case 0x7e: // LD A,(IY+d)
			return void(this.a = this.read(this.disp(this.iyh, this.iyl)));
		case 0x84: // ADD A,IYH (undefined operation)
			return void this.add(this.iyh);
		case 0x85: // ADD A,IYL (undefined operation)
			return void this.add(this.iyl);
		case 0x86: // ADD A,(IY+d)
			return void this.add(this.read(this.disp(this.iyh, this.iyl)));
		case 0x8c: // ADC A,IYH (undefined operation)
			return void this.adc(this.iyh);
		case 0x8d: // ADC A,IYL (undefined operation)
			return void this.adc(this.iyl);
		case 0x8e: // ADC A,(IY+d)
			return void this.adc(this.read(this.disp(this.iyh, this.iyl)));
		case 0x94: // SUB IYH (undefined operation)
			return void this.sub(this.iyh);
		case 0x95: // SUB IYL (undefined operation)
			return void this.sub(this.iyl);
		case 0x96: // SUB (IY+d)
			return void this.sub(this.read(this.disp(this.iyh, this.iyl)));
		case 0x9c: // SBC A,IYH (undefined operation)
			return void this.sbc(this.iyh);
		case 0x9d: // SBC A,IYL (undefined operation)
			return void this.sbc(this.iyl);
		case 0x9e: // SBC A,(IY+d)
			return void this.sbc(this.read(this.disp(this.iyh, this.iyl)));
		case 0xa4: // AND IYH (undefined operation)
			return void this.and(this.iyh);
		case 0xa5: // AND IYL (undefined operation)
			return void this.and(this.iyl);
		case 0xa6: // AND (IY+d)
			return void this.and(this.read(this.disp(this.iyh, this.iyl)));
		case 0xac: // XOR IYH (undefined operation)
			return void this.xor(this.iyh);
		case 0xad: // XOR IYL (undefined operation)
			return void this.xor(this.iyl);
		case 0xae: // XOR (IY+d)
			return void this.xor(this.read(this.disp(this.iyh, this.iyl)));
		case 0xb4: // OR IYH (undefined operation)
			return void this.or(this.iyh);
		case 0xb5: // OR IYL (undefined operation)
			return void this.or(this.iyl);
		case 0xb6: // OR (IY+d)
			return void this.or(this.read(this.disp(this.iyh, this.iyl)));
		case 0xbc: // CP IYH (undefined operation)
			return void this.cp(this.iyh);
		case 0xbd: // CP IYL (undefined operation)
			return void this.cp(this.iyl);
		case 0xbe: // CP (IY+d)
			return void this.cp(this.read(this.disp(this.iyh, this.iyl)));
		case 0xcb:
			v = this.disp(this.iyh, this.iyl);
			switch (this.fetch()) {
			case 0x06: // RLC (IY+d)
				return void this.write(v, this.rlc(this.read(v)));
			case 0x0e: // RRC (IY+d)
				return void this.write(v, this.rrc(this.read(v)));
			case 0x16: // RL (IY+d)
				return void this.write(v, this.rl(this.read(v)));
			case 0x1e: // RR (IY+d)
				return void this.write(v, this.rr(this.read(v)));
			case 0x26: // SLA (IY+d)
				return void this.write(v, this.sla(this.read(v)));
			case 0x2e: // SRA (IY+d)
				return void this.write(v, this.sra(this.read(v)));
			case 0x3e: // SRL (IY+d)
				return void this.write(v, this.srl(this.read(v)));
			case 0x46: // BIT 0,(IY+d)
				return void this.bit(0, this.read(v));
			case 0x4e: // BIT 1,(IY+d)
				return void this.bit(1, this.read(v));
			case 0x56: // BIT 2,(IY+d)
				return void this.bit(2, this.read(v));
			case 0x5e: // BIT 3,(IY+d)
				return void this.bit(3, this.read(v));
			case 0x66: // BIT 4,(IY+d)
				return void this.bit(4, this.read(v));
			case 0x6e: // BIT 5,(IY+d)
				return void this.bit(5, this.read(v));
			case 0x76: // BIT 6,(IY+d)
				return void this.bit(6, this.read(v));
			case 0x7e: // BIT 7,(IY+d)
				return void this.bit(7, this.read(v));
			case 0x86: // RES 0,(IY+d)
				return void this.write(v, this.constructor.res(0, this.read(v)));
			case 0x8e: // RES 1,(IY+d)
				return void this.write(v, this.constructor.res(1, this.read(v)));
			case 0x96: // RES 2,(IY+d)
				return void this.write(v, this.constructor.res(2, this.read(v)));
			case 0x9e: // RES 3,(IY+d)
				return void this.write(v, this.constructor.res(3, this.read(v)));
			case 0xa6: // RES 4,(IY+d)
				return void this.write(v, this.constructor.res(4, this.read(v)));
			case 0xae: // RES 5,(IY+d)
				return void this.write(v, this.constructor.res(5, this.read(v)));
			case 0xb6: // RES 6,(IY+d)
				return void this.write(v, this.constructor.res(6, this.read(v)));
			case 0xbe: // RES 7,(IY+d)
				return void this.write(v, this.constructor.res(7, this.read(v)));
			case 0xc6: // SET 0,(IY+d)
				return void this.write(v, this.constructor.set(0, this.read(v)));
			case 0xce: // SET 1,(IY+d)
				return void this.write(v, this.constructor.set(1, this.read(v)));
			case 0xd6: // SET 2,(IY+d)
				return void this.write(v, this.constructor.set(2, this.read(v)));
			case 0xde: // SET 3,(IY+d)
				return void this.write(v, this.constructor.set(3, this.read(v)));
			case 0xe6: // SET 4,(IY+d)
				return void this.write(v, this.constructor.set(4, this.read(v)));
			case 0xee: // SET 5,(IY+d)
				return void this.write(v, this.constructor.set(5, this.read(v)));
			case 0xf6: // SET 6,(IY+d)
				return void this.write(v, this.constructor.set(6, this.read(v)));
			case 0xfe: // SET 7,(IY+d)
				return void this.write(v, this.constructor.set(7, this.read(v)));
			default:
				this.undefsize = 4;
				if (this.undef)
					this.undef(this.arg);
				return;
			}
		case 0xe1: // POP IY
			return void([this.iyl, this.iyh] = [this.pop(), this.pop()]);
		case 0xe3: // EX (SP),IY
			v = this.read(this.sp);
			this.write(this.sp, this.iyl);
			this.iyl = v;
			v = this.read1(this.sp);
			this.write1(this.sp, this.iyh);
			this.iyh = v;
			return;
		case 0xe5: // PUSH IY
			this.push(this.iyh);
			this.push(this.iyl);
			return;
		case 0xe9: // JP (IY)
			return void(this.pc = this.iyl | this.iyh << 8);
		case 0xf9: // LD SP,IY
			return void(this.sp = this.iyl | this.iyh << 8);
		default:
			this.undefsize = 2;
			if (this.undef)
				this.undef(this.arg);
			return;
		}
	}

	disp(h, l) {
		const d = this.fetch();
		return (l | h << 8) + d - (d << 1 & 0x100) & 0xffff;
	}

	jr(cond) {
		const d = this.fetch();
		if (cond) this.pc = this.pc + d - (d << 1 & 0x100) & 0xffff;
	}

	ret(cond) {
		if (cond) this.pc = this.pop() | this.pop() << 8;
	}

	jp(cond) {
		const addr = this.fetch() | this.fetch() << 8;
		if (cond) this.pc = addr;
	}

	call(cond) {
		const addr = this.fetch() | this.fetch() << 8;
		if (cond) this.rst(addr);
	}

	rst(addr) {
		this.push(this.pc >>> 8);
		this.push(this.pc & 0xff);
		this.pc = addr;
	}

	push(r) {
		this.sp = this.sp - 1 & 0xffff;
		this.write(this.sp, r);
	}

	pop() {
		const r = this.read(this.sp);
		this.sp = this.sp + 1 & 0xffff;
		return r;
	}

	add(r) {
		const v = Z80.aAdd[0][r][this.a];
		this.f = v >>> 8;
		this.a = v & 0xff;
	}

	adc(r) {
		const v = Z80.aAdd[this.f & 1][r][this.a];
		this.f = v >>> 8;
		this.a = v & 0xff;
	}

	sub(r) {
		const v = Z80.aSub[0][r][this.a];
		this.f = v >>> 8;
		this.a = v & 0xff;
	}

	sbc(r) {
		const v = Z80.aSub[this.f & 1][r][this.a];
		this.f = v >>> 8;
		this.a = v & 0xff;
	}

	and(r) {
		this.f = Z80.fLogic[this.a &= r] | 0x10;
	}

	xor(r) {
		this.f = Z80.fLogic[this.a ^= r];
	}

	or(r) {
		this.f = Z80.fLogic[this.a |= r];
	}

	cp(r) {
		this.f = Z80.aSub[0][r][this.a] >>> 8;
	}

	inc(r) {
		const v = Z80.aAdd[0][1][r];
		this.f = this.f & 1 | v >>> 8 & 0xd6;
		return v & 0xff;
	}

	dec(r) {
		const v = Z80.aSub[0][1][r];
		this.f = this.f & 1 | v >>> 8 & 0xd6;
		return v & 0xff;
	}

	rlca() {
		const v = Z80.aRl[this.a >>> 7][this.a];
		this.f = this.f & 0xc4 | v >>> 8 & 1;
		this.a = v & 0xff;
	}

	rrca() {
		const v = Z80.aRr[this.a & 1][this.a];
		this.f = this.f & 0xc4 | v >>> 8 & 1;
		this.a = v & 0xff;
	}

	rla() {
		const v = Z80.aRl[this.f & 1][this.a];
		this.f = this.f & 0xc4 | v >>> 8 & 1;
		this.a = v & 0xff;
	}

	rra() {
		const v = Z80.aRr[this.f & 1][this.a];
		this.f = this.f & 0xc4 | v >>> 8 & 1;
		this.a = v & 0xff;
	}

	incbc() {
		const v = (this.c | this.b << 8) + 1 & 0xffff;
		this.c = v & 0xff;
		this.b = v >>> 8;
	}

	decbc() {
		const v = (this.c | this.b << 8) - 1 & 0xffff;
		this.c = v & 0xff;
		this.b = v >>> 8;
		return v;
	}

	incde() {
		const v = (this.e | this.d << 8) + 1 & 0xffff;
		this.e = v & 0xff;
		this.d = v >>> 8;
	}

	decde() {
		const v = (this.e | this.d << 8) - 1 & 0xffff;
		this.e = v & 0xff;
		this.d = v >>> 8;
	}

	inchl() {
		const v = (this.l | this.h << 8) + 1 & 0xffff;
		this.l = v & 0xff;
		this.h = v >>> 8;
	}

	dechl() {
		const v = (this.l | this.h << 8) - 1 & 0xffff;
		this.l = v & 0xff;
		this.h = v >>> 8;
	}

	rlc(r) {
		const v = Z80.aRl[r >>> 7][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	rrc(r) {
		const v = Z80.aRr[r & 1][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	rl(r) {
		const v = Z80.aRl[this.f & 1][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	rr(r) {
		const v = Z80.aRr[this.f & 1][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	sla(r) {
		const v = Z80.aRl[0][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	sra(r) {
		const v = Z80.aRr[r >>> 7][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	srl(r) {
		const v = Z80.aRr[0][r];
		this.f = v >>> 8;
		return v & 0xff;
	}

	bit(b, r) {
		this.f = this.f & 1 | 0x10 | ~r >>> b << 6 & 0x40;
	}

	static set(b, r) {
		return r | 1 << b;
	}

	static res(b, r) {
		return r & ~(1 << b);
	}

	ldi() {
		this.write(this.e | this.d << 8, this.read(this.l | this.h << 8));
		this.incde();
		this.inchl();
		this.f = this.f & 0xc1 | (this.decbc() !== 0) << 2;
	}

	ldd() {
		this.write(this.e | this.d << 8, this.read(this.l | this.h << 8));
		this.decde();
		this.dechl();
		this.f = this.f & 0xc1 | (this.decbc() !== 0) << 2;
	}

	cpi() {
		this.f = this.f & 1 | Z80.aSub[0][this.read(this.l | this.h << 8)][this.a] >>> 8 & 0xd2;
		this.inchl();
		this.f |= (this.decbc() !== 0) << 2;
	}

	cpd() {
		this.f = this.f & 1 | Z80.aSub[0][this.read(this.l | this.h << 8)][this.a] >>> 8 & 0xd2;
		this.dechl();
		this.f |= (this.decbc() !== 0) << 2;
	}

	ini() {
		this.write(this.l | this.h << 8, this.ioread(this.b, this.c));
		this.f = this.f & 1 | 2 | ((this.b = this.b - 1 & 0xff) === 0) << 6;
		this.inchl();
	}

	ind() {
		this.write(this.l | this.h << 8, this.ioread(this.b, this.c));
		this.f = this.f & 1 | 2 | ((this.b = this.b - 1 & 0xff) === 0) << 6;
		this.dechl();
	}

	outi() {
		this.iowrite(this.b, this.c, this.read(this.l | this.h << 8));
		this.f = this.f & 1 | 2 | ((this.b = this.b - 1 & 0xff) === 0) << 6;
		this.inchl();
	}

	outd() {
		this.iowrite(this.b, this.c, this.read(this.l | this.h << 8));
		this.f = this.f & 1 | 2 | ((this.b = this.b - 1 & 0xff) === 0) << 6;
		this.dechl();
	}

	ioread(h, l) {
		const page = this.iomap[h];
		return !page.read ? page.base[l] : page.read(l | h << 8, this.arg);
	}

	iowrite(h, l, data) {
		const page = this.iomap[h];
		if (!page.write)
			page.base[l] = data;
		else
			page.write(l | h << 8, data, this.arg);
	}
}

void function () {
	let f, i, j, k, r;

	Z80.aAdd = []; // [2][0x100][0x100];
	Z80.aSub = []; // [2][0x100][0x100];
	Z80.aDaa = []; // [8][0x100];
	Z80.aRl = []; // [2][0x100];
	Z80.aRr = []; // [2][0x100];
	Z80.fLogic = new Uint8Array(0x100);

	for (i = 0; i < 2; i++) {
		Z80.aAdd[i] = [];
		for (j = 0; j < 0x100; j++)
			Z80.aAdd[i][j] = new Uint16Array(0x100);
	}
	for (i = 0; i < 2; i++) {
		Z80.aSub[i] = [];
		for (j = 0; j < 0x100; j++)
			Z80.aSub[i][j] = new Uint16Array(0x100);
	}
	for (i = 0; i < 8; i++)
		Z80.aDaa[i] = new Uint16Array(0x100);
	for (i = 0; i < 2; i++)
		Z80.aRl[i] = new Uint16Array(0x100);
	for (i = 0; i < 2; i++)
		Z80.aRr[i] = new Uint16Array(0x100);

	for (i = 0; i < 2; i++)
		for (j = 0; j < 0x100; j++)
			for (k = 0; k < 0x100; k++) {
				r = j + k + i & 0xff;
				const v = j & k & ~r | ~j & ~k & r;
				const c = j & k | k & ~r | ~r & j;
				f = r & 0x80 | !r << 6 | c << 1 & 0x10 | v >>> 5 & 4 | c >>> 7 & 1;
				Z80.aAdd[i][k][j] = r | f << 8;
			}
	for (i = 0; i < 2; i++)
		for (j = 0; j < 0x100; j++)
			for (k = 0; k < 0x100; k++) {
				r = j - k - i & 0xff;
				const v = j & ~k & ~r | ~j & k & r;
				const c = ~j & k | k & r | r & ~j;
				f = r & 0x80 | !r << 6 | c << 1 & 0x10 | v >>> 5 & 4 | 2 | c >>> 7 & 1;
				Z80.aSub[i][k][j] = r | f << 8;
			}
	for (i = 0; i < 0x100; i++) {
		f = i ^ i >>> 4;
		f ^= f >>> 2;
		f ^= f >>> 1;
		Z80.fLogic[i] = i & 0x80 | !i << 6 | ~f << 2 & 4;
	}
	for (i = 0; i < 8; i++)
		for (j = 0; j < 0x100; j++) {
			f = i << 2 & 0x10 | i & 3;
			r = j;
			if ((f & 2) !== 0) {
				if ((f & 0x10) !== 0 && (r & 0x0f) > 5 || (r & 0x0f) > 9) {
					r = r - 6 & 0xff;
					f |= 0x10;
				}
				if ((f & 1) !== 0 && (r & 0xf0) > 0x50 || (r & 0xf0) > 0x90) {
					r = r - 0x60 & 0xff;
					f |= 1;
				}
			}
			else {
				if ((f & 0x10) !== 0 && (r & 0x0f) < 4 || (r & 0x0f) > 9) {
					if ((r += 6) >= 0x100) {
						r &= 0xff;
						f |= 0x01;
					}
					f |= 0x10;
				}
				if ((f & 1) !== 0 && (r & 0xf0) < 0x40 || (r & 0xf0) > 0x90) {
					r = r + 0x60 & 0xff;
					f |= 1;
				}
			}
			Z80.aDaa[i][j] = r | f << 8 | Z80.fLogic[r] << 8;
		}
	for (i = 0; i < 2; i++)
		for (j = 0; j < 0x100; j++) {
			r = j << 1 | i;
			Z80.aRl[i][j] = r | Z80.fLogic[r & 0xff] << 8;
		}
	for (i = 0; i < 2; i++)
		for (j = 0; j < 0x100; j++) {
			r = j >>> 1 | i << 7;
			Z80.aRr[i][j] = r | (Z80.fLogic[r] | j & 1) << 8;
		}
}();

