...
class CrazyBalloon {
	...
	xfer8x8(data, p, k) {
		const q = this.ram[k + 0x400] << 6 | this.ram[k + 0x800] << 14 & 0x3c000;
	...
	xfer32x32(data, dst, src) {
		const color = this.rgb[src >>> 4];
		let collision = 0;

		this.io[2] = this.io[2] & 0xf0 | 0x07;
		if ((dst & 0xff) === 224 || (dst & 0xff00) > 224 * 256 || src >>> 4 === 15)
			return;
		src = src << 10 & 0x3c00;
