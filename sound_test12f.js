...
function success(zip) {
	SND = new Uint8Array(zip.files['rp1_snd0.bin'].inflate().split('').map(c => c.charCodeAt(0))).addBase();
	MCU = new Uint8Array(zip.files['cus64-64a1.mcu'].inflate().split('').map(c => c.charCodeAt(0))).addBase();
	VOI = new Uint8Array(zip.files['rp_voi-0.bin'].inflate().split('').map(c => c.charCodeAt(0))).addBase();
	init({
