...
class PacAndPal {
	constructor() {
		...
		for (let i = 0; i < 0x10; i++)
			this.cpu[1].memorymap[0xf0 + i].base = PRG2.base[i];
	...
	updateStatus() {
		...
		if (this.fDIPSwitchChanged) {
			...
			case 1:
				...
				break;
			case 2:
				this.port[0x15] = this.port[0x15] & ~0x0c | 4;
				break;
			case 3:
				this.port[0x15] = this.port[0x15] & ~0x0c | 8;
				break;
			case 5:
				this.port[0x15] |= 0x0c;
				break;
			...
			case 'A':
				...
				break;
			case 'B':
				this.port[0x14] = this.port[0x14] & ~0x0c | 0x04;
				break;
			case 'C':
				this.port[0x14] = this.port[0x14] & ~0x0c | 0x08;
				break;
			case 'D':
				this.port[0x14] |= 0x0c;
				break;
	...
	updateInput() {
		...
		if (!this.fPortTest) {
			this.ram.set(this.port.subarray(0, 8), 0x2400);
	...
	convertOBJ() {
		...
		for (let i = 0; i < 0x100; i++) {
			...
			this.objcolor[i] = idx === 0x0f ? 0xffffffff : idx < 0x02 ? this.rgb[idx] | 0xff000000 : this.rgb[idx];
		}
	...
	xfer16x16(data, dst, src) {
		...
		for (let i = 16; i !== 0; dst += 256 - 16, --i)
			for (let j = 16; j !== 0; dst++, --j)
				if ((px = this.objcolor[idx + this.obj[src++]]) !== 0xffffffff && ((px & 0xff000000) !== 0 || (data[dst] & 0xff000000) === 0))
					data[dst] = px;
	}
