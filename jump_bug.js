...
class JumpBug {
	constructor() {
		...
		for (let i = 0; i < 0x10; i++)
			this.cpu.memorymap[0xb0 + i].read = addr => {
		...
		this.cpu.memorymap[0x58].write = (addr, data) => sound[0].write(this.psg.addr, data);
		this.cpu.memorymap[0x59].write = (addr, data) => this.psg.addr = data;
		for (let i = 0; i < 8; i++) {
			this.cpu.memorymap[0x60 + i].read = () => this.in[0];
			this.cpu.memorymap[0x60 + i].write = (addr, data, game) => {
				game.mmo[addr & 0xff] = data & 1;
				game.bank = game.mmo[2] | game.mmo[3] << 1 | game.mmo[6] << 2;
			};
			this.cpu.memorymap[0x68 + i].read = () => this.in[1];
			this.cpu.memorymap[0x68 + i].write = (addr, data) => {
				switch (addr & 7) {
				case 3: // BOMB
					if ((data & 1) !== 0)
						this.se[0].start = true;
					else
						this.se[0].stop = true;
					break;
				case 5: // SHOT
					if ((data & 1) !== 0)
						this.se[1].start = true;
					else
						this.se[1].stop = true;
					break;
				}
			...
			this.cpu.memorymap[0x70 + i].write = (addr, data, game) => {
				switch (addr & 7) {
				case 1:
	...
	convertBG() {
		for (let p = 0, q = 0, i = 256; i !== 0; q += 8, --i)
			for (let j = 7; j >= 0; --j)
				for (let k = 7; k >= 0; --k)
					this.bg[p++] = BG[q + k + 0x1800] >>> j & 1 | BG[q + k] >>> j << 1 & 2;
		for (let p = 0x8000, q = 0x800, i = 512; i !== 0; q += 8, --i)
			for (let j = 7; j >= 0; --j)
				for (let k = 7; k >= 0; --k)
					this.bg[p++] = BG[q + k + 0x1800] >>> j & 1 | BG[q + k] >>> j << 1 & 2;
		for (let p = 0, i = 7; i !== 0; p += 0x10000, --i)
			this.bg.copyWithin(p + 0x10000, p, p + 0x10000);
		for (let p = 0, i = 0; i < 8; i++)
			for (let j = 0x10000; j !== 0; p++, --j)
	...
	convertOBJ() {
		for (let p = 0, q = 0, i = 64; i !== 0; q += 32, --i) {
			for (let j = 7; j >= 0; --j) {
				for (let k = 7; k >= 0; --k)
					this.obj[p++] = BG[q + k + 0x1800 + 16] >>> j & 1 | BG[q + k + 16] >>> j << 1 & 2;
				for (let k = 7; k >= 0; --k)
					this.obj[p++] = BG[q + k + 0x1800] >>> j & 1 | BG[q + k] >>> j << 1 & 2;
			}
			for (let j = 7; j >= 0; --j) {
				for (let k = 7; k >= 0; --k)
					this.obj[p++] = BG[q + k + 0x1800 + 24] >>> j & 1 | BG[q + k + 24] >>> j << 1 & 2;
				for (let k = 7; k >= 0; --k)
					this.obj[p++] = BG[q + k + 0x1800 + 8] >>> j & 1 | BG[q + k + 8] >>> j << 1 & 2;
			}
		}
		for (let p = 0x8000, q = 0x800, i = 128; i !== 0; q += 32, --i) {
			for (let j = 7; j >= 0; --j) {
				for (let k = 7; k >= 0; --k)
					this.obj[p++] = BG[q + k + 0x1800 + 16] >>> j & 1 | BG[q + k + 16] >>> j << 1 & 2;
				for (let k = 7; k >= 0; --k)
	...
	xfer8x8(data, p, k, i) {
		let q = (this.ram[k] | this.ram[0xc01 + i * 2] << 10) << 6 & 0x73fc0;

		if (this.mmo[4] && (this.ram[k] & 0xc0) === 0x80)
			q = (this.ram[k] & 0x3f | this.bank << 6 | 0x200 | this.ram[0xc01 + i * 2] << 10) << 6 & 0x7ffc0;
