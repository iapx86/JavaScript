...
//				this.port[0x14] &= ~4;
	...
	updateInput() {
		...
		if (!this.fPortTest) {
			if (this.ram[0x2c0f] === 0x0e) {
				this.ram.set(this.key, 0x2c01);
				this.ram.set(this.key, 0x2c11);
			}
			else {
				this.ram.set(this.port.subarray(4, 8), 0x2c04);
				this.ram.set(this.port.subarray(0x10, 0x18), 0x2c10);
				if (this.ram[0x2c08] <= 8)
	...
	triggerA(fDown) {
		if (fDown)
			this.port[8 - this.ram[0x2c08] & 0x1f] |= 3;
		else
			this.port[8 - this.ram[0x2c08] & 0x1f] &= ~2;
	}
