...
const keydown = e => {
	...
	case 'Space':
		return game.keyboard.fifo.push(0x1d); // Enter
	case 'ArrowLeft':
...
const keyup = e => {
	...
	case 'Space':
		return game.keyboard.fifo.push(0x80 | 0x1d); // Enter
	case 'ArrowLeft':
...
import {ROM, DISK1} from "./dist/quarth_x68.png.js";

window.addEventListener('load', () => expand(ROM).then(ROM => {
	game = new X68000(), game.rom.set(ROM);
}).then(() => expand(DISK1)).then(disk => {
	game.fdd.command.push({unit: 0, disk});
	canvas.addEventListener('click', () => void game.keyboard.fifo.push(0x63, 0x80 | 0x63));
