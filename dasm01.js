	...
	readFileSync(tablefile, 'utf-8').split('\n').map(l => l.trimEnd()).forEach(line => {
		...
		case 't':
			...
			for (let i = base; i < base + size * 2; i += 2)
				attrib.fill('P'.charCodeAt(0), i, i + 2), jumplabel[buffer[i] << 8 | buffer[i + 1]] = true;
			return void(noentry = false);
		case 'u':
			base = parseInt(words[1], 16), size = words.length > 2 ? parseInt(words[2], 10) : 1;
			for (let i = base; i < base + size * 2; i += 2)
				attrib.fill('P'.charCodeAt(0), i, i + 2), label[buffer[i] << 8 | buffer[i + 1]] = true;
			return;
		case 'v':
			base = parseInt(words[1], 16), size = words.length > 2 ? parseInt(words[2], 10) : 1;
...
for (location = start; location < end;) {
	...
	case 'S':
		...
		if (label[base])
			out += `L${x4(base)}`;
		out += `\tfcc\t'${String.fromCharCode(fetch())}`;
